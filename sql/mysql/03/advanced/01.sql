...
from buytbl
group by userid; -- userId를 기준으로 한다. 
...
select userid as 'userID', avg(amount) as '평균 구매 개수'
from buytbl
group by userid;
...
select name, height 
from usertbl
where height = (select max(height) from usertbl) -- 서브 쿼리
	or height = (select min(height) from usertbl);
    
select count(mobile1) as '유대폰이 있는 사용자'
from usertbl;
...
SELECT ROUND(LifeExpectancy, 1) as 'LifeExpectancy' FROM country;
...
-- employees 테이블에서 페이지네이션으로 페이지를 추출하려고 한다. 다음 조건하에서 8번 페이지의 데이터를 출력하세요. : 입사일을 내림차순으로 정렬한다. 한 페이지당 20명의 정보를 출력한다.
select * from employees 
...
-- employees db에서 각 부서별 재직자의 수를 구하시오. 부서 번호로 구분하고, 부서 번호로 오름 차순 정렬하여 출력한다.
