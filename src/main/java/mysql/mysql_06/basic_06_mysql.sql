-- 1. sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl1을 생성하고, 
...
-- 2. sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl2를 생성하고, 
--    자동 생성된 인덱스 목록을 확인하세요.
...
-- 3. sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl3을 생성하고, 
--    자동 생성된 인덱스 목록을 확인하세요.
...
-- 4. sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl4를 생성하고, 
--    자동 생성된 인덱스 목록을 확인하세요.
...
-- 5. sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl5를 생성하고, 
--    자동 생성된 인덱스 목록을 확인하세요.
...
-- 7. ALTER를 사용하여 usertbl에서 PRIMARY KEY 제약조건을 제거하고,
